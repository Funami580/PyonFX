...
_libass_path = os.path.join(Path(__file__).parent.parent.parent, "libass/libass/.libs/")
_libass_file = [x for x in os.listdir(_libass_path)
                if x.lower().startswith("libass") and x.lower().endswith((".so", ".dll", ".dylib"))][0]
_libass_path = os.path.join(_libass_path, _libass_file)
...
class Renderer(ctypes.Structure):
    ...
    def get_glyph_info(self, track, now):
        if not self._fonts_set:
            raise RuntimeError("set_fonts before rendering")
        head = _libass.ass_glyph_info(ctypes.byref(self),
                                      ctypes.byref(track),
                                      Renderer.timedelta_to_ms(now))
        return head
...
_libass.ass_glyph_info.argtypes = [
    ctypes.POINTER(Renderer),
    ctypes.POINTER(Track),
    ctypes.c_longlong
]
_libass.ass_glyph_info.restype = ctypes.POINTER(GlyphData)
